...
struct BoardConfig_t {
    ...
    // MKS Mini working defaults (confirmed calibration ok on user's hardware):
    // - brake_resistance 2.0 Ohm (MKS Mini external brake resistor)
    // - brake enabled
    // - OVP 30V (safe margin above 24V bus; default 1.07*HW_VERSION_VOLTAGE=59.92 is too high for 24V ops)
    // - UVP 4V (permissive low limit)
    float max_regen_current = 0.0f;
    float brake_resistance = 2.0f;                              // MKS Mini: external 2Ω brake resistor
    bool enable_brake_resistor = true;                          // MKS Mini default: on
    float dc_bus_undervoltage_trip_level = 4.0f;                //<! [V] minimum voltage below which the motor stops operating
    float dc_bus_overvoltage_trip_level = 30.0f;                //<! [V] maximum voltage above which the motor stops operating.
    ...
    float dc_max_negative_current = -0.1f; // Max current [A] the power supply can sink. MKS Mini default (was -0.01f stock).
