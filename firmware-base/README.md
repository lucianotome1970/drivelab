...
**Root cause of the "software jump" failure — reopened (2026-08-01):** systematic on-device debugging (see `firmware-base/tools/enter_dfu.py` below) confirmed the `EnterDfu` command *does* reset the board — it disappears from USB as expected — but the ROM bootloader's USB peripheral then fails to enumerate after a **warm** jump. The magic value in `.noinit` RAM survives the reset fine (an RTC-backup-register trigger, considered earlier, would not help — that was never the problem), and `SW1 → DFU` works because it's a **cold boot**. ⚠️ The earlier explanation — *"this specific board's VBUS/5V path is physically burnt"* — was **wrong**: a **brand-new board behaves identically** (plugging only the board's USB never powers or enumerates it), so this is the board's design (the USB port doesn't feed the logic), not a burnt unit. The actual cause of the warm-jump failure is **open again** — leading hypothesis: the ROM bootloader wants USB **VBUS-sense**, which this board's USB port doesn't wire (our app's TinyUSB runs with VBUS sensing off), but this hasn't been re-diagnosed. Practical impact is nil: the `SW1 → DFU` cold-boot fallback is the path we use.
...
>
> **Known limitations:**
> 1. The **fully hands-off automatic jump does not work** — `EnterDfu` resets the board correctly, but the STM32 ROM bootloader's USB does not enumerate after a warm jump. ⚠️ The earlier "this board's VBUS/5V path is physically burnt" reason was **wrong** — a brand-new board behaves identically (never powers or enumerates from USB alone; it's the board's design, not a defect). The real cause of the warm-jump failure is **still open** (leading hypothesis: the ROM bootloader's USB VBUS-sense isn't wired on the USB port). The `SW1 → DFU` cold-boot fallback works and is the path in use; the RTC-backup-register trigger would not fix this — the magic already survives the reset today.
...
**Causa raiz da falha do "salto por software" — reaberta (2026-08-01):** uma depuração sistemática no próprio dispositivo (ver `firmware-base/tools/enter_dfu.py` abaixo) confirmou que o comando `EnterDfu` *de fato* reseta a placa — ela some da USB como esperado — mas depois a USB do bootloader da ROM não enumera após um salto **a quente**. O valor mágico na RAM `.noinit` sobrevive ao reset (um gatilho via registrador de backup do RTC, cogitado antes, não resolveria isso — nunca foi o problema), e o `SW1 → DFU` funciona por ser um **boot a frio**. ⚠️ A explicação anterior — *"o caminho VBUS/5V desta placa está fisicamente queimado"* — estava **errada**: uma **placa nova se comporta igual** (plugar só a USB da placa nunca a alimenta nem enumera), então é o projeto da placa (a porta USB não alimenta a lógica), não uma unidade queimada. A causa real da falha do salto a quente está **aberta de novo** — hipótese principal: o bootloader da ROM quer o **VBUS-sense** da USB, que a porta USB desta placa não fia (o TinyUSB do nosso app roda com VBUS-sense desligado), mas isso não foi re-diagnosticado. Impacto prático é nulo: o fallback `SW1 → DFU` (boot a frio) é o caminho que usamos.
...
> 1. O **salto automático totalmente sem intervenção não funciona** — o `EnterDfu` reseta a placa corretamente, mas a USB do bootloader da ROM da STM32 não enumera depois de um salto a quente. ⚠️ A razão anterior "o caminho VBUS/5V desta placa está fisicamente queimado" estava **errada** — uma placa nova se comporta igual (nunca alimenta nem enumera só pela USB; é o projeto da placa, não um defeito). A causa real da falha do salto a quente ainda está **aberta** (hipótese principal: o VBUS-sense da USB do bootloader da ROM não é fiado na porta USB). O fallback `SW1 → DFU` (boot a frio) funciona e é o caminho em uso; o gatilho via registrador de backup do RTC não resolveria isso — o mágico já sobrevive ao reset hoje.
