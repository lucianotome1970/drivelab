...
>
> **Update (2026-08-01): the fully automatic zero-touch jump now WORKS** — the warm-jump bug was the interrupt mask (`PRIMASK`) left set before entering the ROM bootloader; fixed in `lib/base_usb/dfu_jump.cpp` (see the root-cause note above). Validated end-to-end at the firmware level via `tools/enter_dfu.py` + `dfu-util` (firmware → `EnterDfu` → `0483:df11` → flash → `:leave` → firmware, no ST-Link). Re-validating the same automatic flow **through DriveLab Studio's Send button** (which now no longer needs the `SW1 → DFU` fallback) is a quick follow-up.
>
> **Known limitations:**
> 1. ~~The **fully hands-off automatic jump does not work**~~ **FIXED (2026-08-01):** the warm jump now enumerates the ROM DFU. Root cause was **`PRIMASK=1`** (interrupts left masked before the jump); the ROM's interrupt-driven USB DFU never fired. Fixed with `__enable_irq()` before the branch (`lib/base_usb/dfu_jump.cpp`); validated end-to-end over the data USB (no ST-Link, no `SW1`). Remaining follow-up: re-run the same automatic flow **through DriveLab Studio's Send** (the manual `SW1 → DFU` fallback stays as a safety net). The earlier "VBUS/5V burnt" and "VBUS-sense" guesses were both wrong.
...
>
> **Atualização (2026-08-01): o salto automático zero-toque agora FUNCIONA** — o bug do salto a quente era a máscara de interrupção (`PRIMASK`) deixada ligada antes de entrar no bootloader da ROM; corrigido em `lib/base_usb/dfu_jump.cpp` (ver a causa raiz acima). Validado ponta a ponta no nível do firmware via `tools/enter_dfu.py` + `dfu-util` (firmware → `EnterDfu` → `0483:df11` → gravação → `:leave` → firmware, sem ST-Link). Re-validar o mesmo fluxo automático **pelo botão Enviar do DriveLab Studio** (que agora não precisa mais do fallback `SW1 → DFU`) é um follow-up rápido.
>
> **Limitações conhecidas:**
> 1. ~~O **salto automático totalmente sem intervenção não funciona**~~ **CORRIGIDO (2026-08-01):** o salto a quente agora enumera o DFU da ROM. A causa era **`PRIMASK=1`** (interrupções mascaradas antes do salto); a USB DFU do ROM, dirigida por interrupção, nunca disparava. Corrigido com `__enable_irq()` antes do branch (`lib/base_usb/dfu_jump.cpp`); validado ponta a ponta pela USB de dados (sem ST-Link, sem `SW1`). Follow-up restante: re-rodar o mesmo fluxo automático **pelo Enviar do DriveLab Studio** (o fallback manual `SW1 → DFU` fica como rede de segurança). Os palpites anteriores de "VBUS/5V queimado" e "VBUS-sense" estavam ambos errados.
