...
public sealed class SimulatorBaseTransport : IBaseTransport
{
    ...
    /// <summary>Valor de fábrica do ajuste. Consulta pura: o simulador NÃO altera o valor
    /// guardado, exatamente como a placa — quem grava é o Salvar.</summary>
    public Task<SettingValue> ReadSettingDefaultAsync(BaseSettingId id)
    {
        var descritor = BaseSettingsSchema.All.FirstOrDefault(d => d.Id == id);
        return Task.FromResult(descritor is null
    ...
    private void ApplySettingsToWheel()
    {
        ...
        // A curva de resposta tambem vale no simulador: arrastar um ponto no grafico muda o volante
        // da tela na hora, em vez de so descobrir o efeito na pista.
        _wheel.ForceCurve = VirtualBase.IdsDaCurva.Select(id => _settings[id].AsDouble).ToArray();
    }
