...
public static class CompositionRoot
{
    ...
    {
        // Agrupado por ASSUNTO, não por "avançado/básico": ajustar o batente exigia pular entre
        // Basic (força/range) e Advanced (amortecimento) — os três moram juntos agora.
        new("Basic", new[]
        {
        ...
        new("Advanced", new[]
        {
            ...
            // Os cinco pontos da CURVA de resposta. Ficam no grupo (e por isso carregam da placa e
            // vao no Salvar), mas quem os edita e o grafico — ver AdvancedTabViewModel.
            BaseSettingId.FfbCurve0,
            BaseSettingId.FfbCurve1,
            BaseSettingId.FfbCurve2,
            BaseSettingId.FfbCurve3,
            BaseSettingId.FfbCurve4,
            BaseSettingId.FfbCurve5,
            BaseSettingId.FfbCurve6,
            BaseSettingId.FfbCurve7,
            BaseSettingId.FfbCurve8,
            BaseSettingId.FfbCurve9,
            BaseSettingId.FfbCurve10,
        }),
        new("Hardware", new[]
        {
            // PRIMEIRO campo da aba, de propósito: é a trava de bring-up. A base sobe DESARMADA;
            // o usuário confere os campos abaixo (placa, encoder, motor) e só então ativa. Se o
            // ajuste estiver errado, desliga e o motor para na hora.
            BaseSettingId.MotorEnable,
    ...
    public static MainWindowViewModel CreateMainWindowViewModel(IBaseTransport? transport = null, bool simulatorMode = false, bool advancedMode = false)
    {
        ...
        var wheelBaseTabs = WheelBaseTabs
            ...
            .Select(t => new PageTab(L.Get($"Tab_{t.Header}"), t.Header switch
            {
                "Hardware" => new HardwareTabViewModel(session, t.Header, t.Ids),
                "Advanced" => new AdvancedTabViewModel(session, t.Header, t.Ids),
        ...
        var dashboard = new DashboardViewModel(session, wheelSession, centerHotkey);
        var baseCard = new BaseViewModel(session);
        // A força total mora no card da base, mas o "Salvar no controlador" fica no dashboard:
        // amarra os dois para que NADA vá à placa antes do Salvar.
        dashboard.AttachPending(baseCard);
        var home = new HomeViewModel(dashboard, pedals, handbrake, baseCard);
        ...
        // Sem conexão → nada é lido. Ver docs/sessoes/HANDOFF.md.
