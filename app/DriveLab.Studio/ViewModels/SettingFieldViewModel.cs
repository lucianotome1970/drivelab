...
public partial class SettingFieldViewModel : ViewModelBase
{
    ...
    /// <summary>Explicação do que o controle faz, com exemplo — exibida no "?" ao lado do rótulo.
    ///
    /// Duas fontes, porque as duas metades do projeto escreveram ajuda por caminhos diferentes:
    /// <c>SettingHelp_{Id}</c> nos resx (traduzível, EN+PT) tem PRIORIDADE, e o texto do schema
    /// (<see cref="SettingDescriptor.Help"/>, só em português) entra como reserva onde ainda não há
    /// chave traduzida. Assim nenhum texto se perde e a tradução continua sendo o caminho preferido.
    /// Vazio nos dois = o "?" some.</summary>
    public string HelpText
    {
        get
        {
            var key = $"SettingHelp_{_descriptor.Id}";
            var text = L.Get(key);
            if (text != key) return text;               // L.Get devolve a própria chave se não achar
            return _descriptor.Help ?? string.Empty;    // reserva: texto do schema
        }
    }

    /// <summary>Há explicação para este campo? (esconde o "?" nos que ainda não têm texto)</summary>
    public bool HasHelp => HelpText.Length > 0;
    ...
    private int _encoderModel = EncoderCatalog.E6b2;
    ...
    {
        _tech = tech;
        _encoderModel = EncoderCatalog.Normalize(modelId);
    ...
    /// <summary>Volta o campo ao valor padrão do schema (grava se conectado). Reserva para quando a
    /// base não souber responder o padrão — ver <see cref="ApplyDefaultFromDevice"/>.</summary>
    public void ResetToDefault() => Value = _descriptor.Default;

    /// <summary>Adota o padrão que a BASE respondeu (report 0x17). Passa pelo mesmo caminho de
    /// qualquer edição — o campo fica marcado como alterado e o Salvar é que grava na placa.
    ///
    /// <para>No MODELO do encoder o valor passa pelo <see cref="EncoderCatalog.Normalize"/>: o
    ...
    public async Task LoadAsync()
    {
        ...
        IsModified = false; // app == base
    }

    /// <summary>Alterado na UI e ainda NÃO enviado à base. Zera ao carregar da placa e ao enviar.</summary>
    public bool IsModified { get; private set; }

    /// <summary>O usuário mexeu neste campo (não dispara em load/eco do device).</summary>
    ...
    public async Task WriteAsync()
    {
        ...
        await _session.WriteSettingAsync(_descriptor.Id, new SettingValue(_descriptor.Type, _descriptor.Clamp(Value)));
    ...
    {
        OnPropertyChanged(nameof(ValueText));
        // O SLIDER lê DisplayValue, não Value — sem esta notificação ele não acompanha quem mudou o
        // valor por outro caminho. Na bancada (2026-08-12) o usuário clicou no chip "70" de Força
        // total: a caixa passou a mostrar 70% e o slider ficou parado no 100, dando a impressão de
        // que o clique não pegou. O valor estava certo o tempo todo; a tela é que contava duas
        // histórias diferentes sobre ele.
        OnPropertyChanged(nameof(DisplayValue));
        UpdatePresetSelection();
        UpdateOptionSelection();
        // NÃO envia para a base aqui. Mexer no controle só altera o valor local e marca pendência;
        // quem envia é o "Salvar no controlador" (SettingsPageViewModel.SaveAsync).
        //
        // Antes cada movimento de slider disparava um WriteSettingAsync. Isso reconfigurava a base ao
        // vivo, com o motor possivelmente ARMADO — e mexer em setting com o motor armado já derrubou
        // a placa neste projeto (aba Hardware, 2026-08-05). Também gerava uma rajada de writes
        // enquanto o usuário arrastava o controle.
        if (!_loading)
        {
            IsModified = true;
