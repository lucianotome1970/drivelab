...
public sealed class HidBaseTransport : IBaseTransport, IDisposable
{
    ...
    public async Task<SettingValue> ReadSettingAsync(BaseSettingId id, TimeSpan timeout) =>
        await RequestValueAsync(id, BaseReportIds.SettingReadRequest, timeout);

    /// <summary>Valor de FÁBRICA do ajuste (report 0x17). Consulta pura: não altera a placa.</summary>
    ...
    // ser exibido como se fosse o valor gravado. O _readGate já serializa tudo, e é por isso que
    // as duas compartilham este caminho em vez de ganharem um cada.
    private async Task<SettingValue> RequestValueAsync(BaseSettingId id, byte reportId, TimeSpan timeout)
    {
        ...
        try
        {
            ...
            await _channel.WriteAsync(Frame(reportId, new SettingReadRequestReport((byte)id, 0).ToBytes()));
