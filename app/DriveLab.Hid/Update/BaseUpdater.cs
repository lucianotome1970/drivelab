...
public sealed class BaseUpdater : IDeviceUpdater
{
    ...
    private string? ResolveDfuUtilPath()
    {
        ...
        // 1) AO LADO DO APP — é o caso do usuário final, que não instala nada à mão. O instalador
        //    do Windows põe o dfu-util aqui; sem esta busca, empacotá-lo não adiantaria e o app
        //    continuaria dependendo de o usuário tê-lo no PATH (constatado na bancada em
        //    2026-08-10: o EnterDfu funcionou, a placa entrou em DFU, e a atualização parou aqui).
        var appDir = AppContext.BaseDirectory;
        if (!string.IsNullOrEmpty(appDir))
        {
            string[] local =
            [
                Path.Combine(appDir, OperatingSystem.IsWindows() ? "dfu-util.exe" : "dfu-util"),
                Path.Combine(appDir, "tools", OperatingSystem.IsWindows() ? "dfu-util.exe" : "dfu-util"),
            ];
        ...
        // 2) instalações típicas de quem desenvolve
        string[] candidates =
        [
            "/opt/homebrew/bin/dfu-util", // Apple Silicon Homebrew
            "/usr/local/bin/dfu-util",    // Intel Homebrew / typical Linux
            @"C:\msys64\mingw64\bin\dfu-util.exe",   // MSYS2 (o ambiente de build deste projeto)
            @"C:\msys64\ucrt64\bin\dfu-util.exe",
        ];
