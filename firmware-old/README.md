...
>
> *Board caveat:* this board **doesn't take logic power from the USB port** — plugging only the board's USB never lights the PWR LED (confirmed on **both this bench unit and a brand-new board**, so it's the board's design — USB is data-only, the logic is powered from the DC bus or the ST-Link — **not** a burnt component). The **D+/D‑ data lines are fine**: it enumerates normally once the logic is powered another way (DC bus, or ST-Link 3.3V, which *does* light the PWR LED). Tip: put the ST-Link on a **USB charger** just for power (it still outputs 3.3V without a data host), freeing the laptop's only USB port for the board's data cable.
...
**Root cause of the "software jump" failure — RESOLVED (2026-08-01):** systematic on-device debugging (SWD + `firmware-base/tools/enter_dfu.py`) confirmed `EnterDfu` resets the board and the CPU **does** reach the ROM bootloader (SWD halt showed `pc` in `0x1FFF_xxxx`, USB clock `OTGFSEN=1`) — but the jump left **`PRIMASK=1`** (global interrupts masked by a `__disable_irq()` that was never undone). The ROM bootloader's USB DFU is **interrupt-driven**, so with IRQs masked its OTG_FS handler never fired and the device never enumerated — while a cold `SW1 → DFU` boot works precisely because it starts with `PRIMASK=0`. **Fix:** re-enable interrupts (`__enable_irq()`) immediately before branching to the bootloader (`lib/base_usb/dfu_jump.cpp`, shared by m5 + m05). Confirmed by SWD (PRIMASK read `0x01` before / `0x00` after) and **validated end-to-end**: firmware → `EnterDfu` → DFU `0483:df11` enumerates in ~0.7 s → `dfu-util` flashes (`File downloaded successfully`) → `:leave` reboots into the firmware, all over the data USB, **no ST-Link, no `SW1` switch, no power-cycle**. (The earlier "VBUS/5V burnt" and "VBUS-sense" guesses were both wrong — the USB clock and jump were fine all along; only the interrupt mask blocked enumeration.)
...
The send flow (`BaseUpdater`) first tries the automatic path: it sends `EnterDfu` over the current HID transport and waits ~8 s for the DFU device to appear. If it doesn't (a fallback for boards/cases where the automatic jump fails — since the 2026-08-01 PRIMASK fix this bench board succeeds automatically), the app falls into a **manual fallback mode**: it keeps exclusive USB access (pauses auto-connect and releases the HID handle) and prompts the user to set **`SW1 → DFU` and power-cycle** the board. A **Continuar/Continue** button re-scans for the DFU device and, once found, flashes it via `dfu-util`; **Cancelar/Cancel** aborts and resumes normal auto-connect. Either path — automatic or manual — flashes over the same data USB cable, no ST-Link.
...
> **Status (2026-07-19): validated end-to-end on hardware ✅** — on the bench MKS ODRIVE-S / STM32F405, flashing the base **through DriveLab Studio** (Send → automatic attempt times out as expected → manual `SW1 → DFU` fallback → `dfu-util`) changed the board's **reported firmware version from 0.1.0 to 0.2.0**, confirmed by re-reading the 0x21 telemetry after the flash. This is the reliable update path on this board today: **app Send → automatic attempt (fell through on this board until the 2026-08-01 PRIMASK fix — the automatic path now succeeds) → `SW1 → DFU` manual fallback prompted by the app → flash**.
>
> **Update (2026-08-01): the fully automatic zero-touch jump now WORKS** — the warm-jump bug was the interrupt mask (`PRIMASK`) left set before entering the ROM bootloader; fixed in `lib/base_usb/dfu_jump.cpp` (see the root-cause note above). Validated end-to-end at the firmware level via `tools/enter_dfu.py` + `dfu-util` (firmware → `EnterDfu` → `0483:df11` → flash → `:leave` → firmware, no ST-Link). **Also validated through DriveLab Studio's Send button (2026-08-01):** flashing 0.3.6 → 0.3.7 ran the automatic path start to finish (USB drops → DFU → flash → reboot) with **no `SW1 → DFU` prompt**; 0x21 telemetry confirmed the board back up as v0.3.7. The manual `SW1 → DFU` fallback now stays only as a safety net.
>
> **Known limitations:**
> 1. ~~The **fully hands-off automatic jump does not work**~~ **FIXED (2026-08-01):** the warm jump now enumerates the ROM DFU. Root cause was **`PRIMASK=1`** (interrupts left masked before the jump); the ROM's interrupt-driven USB DFU never fired. Fixed with `__enable_irq()` before the branch (`lib/base_usb/dfu_jump.cpp`); validated end-to-end over the data USB (no ST-Link, no `SW1`) **and through DriveLab Studio's Send (0.3.6 → 0.3.7, fully automatic, no `SW1`)**; the manual fallback stays as a safety net. The earlier "VBUS/5V burnt" and "VBUS-sense" guesses were both wrong.
...
>
> *Ressalva desta placa:* esta placa **não tira a alimentação da lógica pela porta USB** — plugar só a USB da placa nunca acende o LED PWR (confirmado **tanto nesta unidade de bancada quanto numa placa nova**, então é o projeto da placa — a USB é só dados, a lógica vem do barramento DC ou do ST-Link — **não** um componente queimado). Os **dados D+/D‑ estão ok**: enumera normal quando a lógica é alimentada por outro caminho (barramento DC, ou o 3.3V do ST-Link, que *acende* o LED PWR). Dica: deixe o ST-Link num **carregador USB** só pra alimentar (ele solta 3.3V mesmo sem host de dados), liberando a única USB do note pro cabo de dados da placa.
...
**Causa raiz da falha do "salto por software" — RESOLVIDA (2026-08-01):** depuração sistemática no dispositivo (SWD + `firmware-base/tools/enter_dfu.py`) confirmou que o `EnterDfu` reseta a placa e a CPU **chega** ao bootloader da ROM (o halt por SWD mostrou `pc` em `0x1FFF_xxxx`, clock da USB `OTGFSEN=1`) — mas o salto deixava **`PRIMASK=1`** (interrupções globais mascaradas por um `__disable_irq()` que nunca era desfeito). A USB DFU do bootloader da ROM é **dirigida por interrupção**, então com as IRQs mascaradas o handler do OTG_FS nunca disparava e o dispositivo nunca enumerava — enquanto o `SW1 → DFU` a frio funciona justamente por nascer com `PRIMASK=0`. **Fix:** reabilitar as interrupções (`__enable_irq()`) imediatamente antes de saltar pro bootloader (`lib/base_usb/dfu_jump.cpp`, compartilhado por m5 + m05). Confirmado por SWD (PRIMASK lia `0x01` antes / `0x00` depois) e **validado ponta a ponta**: firmware → `EnterDfu` → DFU `0483:df11` enumera em ~0,7 s → `dfu-util` grava (`File downloaded successfully`) → `:leave` reinicia no firmware, tudo pela USB de dados, **sem ST-Link, sem `SW1`, sem power-cycle**. (Os palpites anteriores de "VBUS/5V queimado" e "VBUS-sense" estavam ambos errados — o clock da USB e o salto sempre estiveram OK; só a máscara de interrupção travava a enumeração.)
...
> **Status (2026-07-19): validado ponta a ponta no hardware ✅** — na MKS ODRIVE-S / STM32F405 de bancada, gravar a base **pelo DriveLab Studio** (Enviar → tentativa automática expira como esperado → fallback manual `SW1 → DFU` → `dfu-util`) mudou a **versão de firmware reportada pela placa de 0.1.0 para 0.2.0**, confirmado relendo a telemetria 0x21 depois da gravação. Esse é o caminho de atualização confiável nesta placa hoje: **Enviar no app → tentativa automática (caía no fallback até o fix de 2026-08-01 — agora o automático funciona nesta placa) → fallback manual `SW1 → DFU` pedido pelo app → gravação**.
>
> **Atualização (2026-08-01): o salto automático zero-toque agora FUNCIONA** — o bug do salto a quente era a máscara de interrupção (`PRIMASK`) deixada ligada antes de entrar no bootloader da ROM; corrigido em `lib/base_usb/dfu_jump.cpp` (ver a causa raiz acima). Validado ponta a ponta no nível do firmware via `tools/enter_dfu.py` + `dfu-util` (firmware → `EnterDfu` → `0483:df11` → gravação → `:leave` → firmware, sem ST-Link). **Também validado pelo botão Enviar do DriveLab Studio (2026-08-01):** gravar 0.3.6 → 0.3.7 rodou a via automática do início ao fim (USB cai → DFU → grava → reinicia), **sem pedir o `SW1 → DFU`**; a telemetria 0x21 confirmou a placa de volta em v0.3.7. O fallback manual `SW1 → DFU` fica agora só como rede de segurança.
>
> **Limitações conhecidas:**
> 1. ~~O **salto automático totalmente sem intervenção não funciona**~~ **CORRIGIDO (2026-08-01):** o salto a quente agora enumera o DFU da ROM. A causa era **`PRIMASK=1`** (interrupções mascaradas antes do salto); a USB DFU do ROM, dirigida por interrupção, nunca disparava. Corrigido com `__enable_irq()` antes do branch (`lib/base_usb/dfu_jump.cpp`); validado ponta a ponta pela USB de dados (sem ST-Link, sem `SW1`) **e pelo Enviar do DriveLab Studio (0.3.6 → 0.3.7, totalmente automático, sem `SW1`)**; o fallback manual fica como rede de segurança. Os palpites anteriores de "VBUS/5V queimado" e "VBUS-sense" estavam ambos errados.
