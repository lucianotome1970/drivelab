# Pinagem e montagem — placa da base (ODrive v3.6 / MKS ODESC / XDrive)

> **PT abaixo · English below**

Referência de **todos os pinos do microcontrolador**, o que cada um faz, o que está livre, e como
ligar os sensores e botões que o DriveLab usa.

**A fonte desta tabela é o código**, não um datasheet: os nomes vêm de
`firmware-base/vendor/odrive-fw/Board/v3/Inc/main.h` e o uso, de onde o firmware lê e escreve. Se
divergir da sua placa, o código manda.

> ⚠️ **O que esta página NÃO sabe:** a posição física de cada pino nos conectores da sua placa. O
> mapa abaixo é `microcontrolador → função`; a ordem dos pinos no header depende do fabricante.
> **Confira sempre a serigrafia da placa** antes de soldar. Um GPIO trocado por uma fase é o fim da
> placa.
