...
> ⛔ **Documento histórico — a arquitetura descrita aqui não existe mais.**
>
> Foi o design de quando a placa ainda não estava em mãos, e os módulos que ele propõe
> (`ffb_engine.h`, `ffb_controller.h`, `pi_controller.h`, `filters.h`, `startup.h`, `ffb_power.h`,
> `cogging.h`) saíram do repositório junto com o firmware anterior. O firmware de hoje é construído
> sobre a base ODrive.
>
> **A ideia central sobreviveu** e é o que vale reler: separar a lógica do silício para poder testá-la
> no PC. É o que os testes de host em `firmware-base/test/` fazem até hoje.
>
> 🇬🇧 **Historical — the architecture below no longer exists.** The modules it proposes left the
> repository with the previous firmware; today's firmware is built on the ODrive base. The core idea
> survived: keep the logic separable from the silicon so it can be tested on a PC — which is what the
> host tests in `firmware-base/test/` still do.
