...
> ⛔ **Historical document — do not use as current status.**
>
> It was written on 2026-08-01, **before** the firmware was rebuilt on an ODrive base. It describes
> the previous architecture: `FocPower`, `FocCurrent`, `m5/main.cpp`, `apply_cfg.h`, `brake_pwm.h`,
> `motor_hal.h` — none of which exist today. Several entries are also simply out of date: the brake
> chopper is listed as pending and has since been **validated under real FFB**, and the encoder CPR
> is listed as hard-coded and is now a setting.
>
> **For current status:** [estado-validado.md](sessoes/estado-validado.md) and [HANDOFF.md](sessoes/HANDOFF.md).
> Kept because the **measurements** in it are real and worth not repeating — the bus voltage scale
> calibration, in particular.
>
> 🇧🇷 **Documento histórico — não use como estado atual.** Foi escrito em 01/08/2026, **antes** de o
> firmware ser refeito sobre a base ODrive, e descreve a arquitetura anterior. Vários itens também
> venceram: o brake chopper aparece como pendente e já foi **validado sob FFB real**, e o CPR do
> encoder aparece como cravado e hoje é um ajuste. **Estado atual:**
> [estado-validado.md](sessoes/estado-validado.md) e [HANDOFF.md](sessoes/HANDOFF.md). Fica no repositório porque as
> **medições** dele são reais e não vale a pena refazer.
