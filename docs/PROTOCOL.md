...
| `0x02` | Out | Command | `[0]=commandId [1]=arg` — **rim / pedals / handbrake** |
| `0x22` | Out | Command | same payload — **base** (the base uses `0x22`, not `0x02`) |
| `0x14` | Out | SettingWrite | Write one setting |
| `0x15` | Out | SettingReadRequest | Ask for one setting |
| `0x17` | Out | SettingDefaultRequest | Ask for a setting's **factory default** (base only) |
| `0x16` | In | SettingValue | Reply to `0x15` **and** to `0x17` |
...
`UInt32` exists because a 21-bit magnetic encoder reports 2,097,152 counts per turn, which does not fit in 16 bits. `encoder_cpr` is the setting that uses it.
...
**SettingDefaultRequest `0x17`** (out, base only) — same payload as `0x15`:
...
It asks for the setting's **factory default**, and the device replies with an ordinary `0x16`. This is a **pure query**: the stored value is not touched and nothing is applied. It exists so a "Defaults" button can show what the firmware would use without committing to it — the user still has to save. Implemented in the base firmware; the app does not send it yet.
...
### 6. Commands (`0x02` rim/pedals, `0x22` base)
...
- **Base** (`BaseCommand`, sent on `0x22`): `Reboot=1, SaveSettings=2, ResetCenter=3, EnterDfu=4, Calibrate=5, SetForceEnabled=6, CalibrateCogging=7, BrakeBench=8, BrakeAuto=9`.
...
| `0x02` | Out | Command | `[0]=commandId [1]=arg` — **aro / pedais / freio de mão** |
| `0x22` | Out | Command | mesmo payload — **base** (a base usa `0x22`, não `0x02`) |
| `0x14` | Out | SettingWrite | Grava um setting |
| `0x15` | Out | SettingReadRequest | Pede um setting |
| `0x17` | Out | SettingDefaultRequest | Pede o **valor de fábrica** do setting (só na base) |
| `0x16` | In | SettingValue | Resposta do `0x15` **e** do `0x17` |
...
O `UInt32` existe porque um encoder magnético de 21 bits reporta 2.097.152 contagens por volta, o que não cabe em 16 bits. O setting que usa esse tipo é o `encoder_cpr`.
...
**SettingDefaultRequest `0x17`** (out, só na base) — mesmo payload do `0x15`:
...
Pede o **valor de fábrica** do setting, e o device responde com um `0x16` comum. É uma **consulta pura**: o valor guardado não é tocado e nada é aplicado. Existe para um botão "Padrão" mostrar o que o firmware usaria sem se comprometer com isso — quem decide continua sendo o usuário, ao salvar. Implementado no firmware da base; o app ainda não envia.
...
### 6. Comandos (`0x02` aro/pedais, `0x22` base)
...
- **Base** (`BaseCommand`, vai no `0x22`): `Reboot=1, SaveSettings=2, ResetCenter=3, EnterDfu=4, Calibrate=5, SetForceEnabled=6, CalibrateCogging=7, BrakeBench=8, BrakeAuto=9`.
